...
socket.onerror = (evt) => document.querySelector('#message').innerText = evt[message];
chatSocket.onerror = (evt) => document.querySelector('#message').innerText = evt[message];
...
// Точка входа в отображение карточки пользователя + комнат + чата
...
// Запрос данных с сервера пользователя + чат
...
// Вывод на страницу содержимого карточки пользователя
function printUserCard(item) {
    ...
    <div class="div">
        <img src="${item.avatar}">
        <br>
        <strong>Сменить аватар:</strong><br>
...
// Функция выгрузки на сервер аватарки пользователя
